...
def get_classification_model(model_name):
    if model_name == "upperbody":
        return tf.keras.models.load_model(os.path.join(main_directory, 'modeller/upperbody/upperbody_acc_train_092_val_073_incV3_299_299.h5'))
    if model_name == "lowerbody":
        return tf.keras.models.load_model(os.path.join(main_directory, 'modeller/lowerbody/lowerbody_acc_train_093_val_088_incV3_299_299.h5'))
    if model_name == "pants":
        return tf.keras.models.load_model(os.path.join(main_directory, 'modeller/lowerbody/pants/pants_type_acc_train_09_val_08_incV3_299_299.h5'))
    if model_name == "accessories":
        return tf.keras.models.load_model(os.path.join(main_directory, 'modeller/accessories/accessories_acc_train_097_val_093_incV3_299_299.h5'))
    if model_name == "shirt_blouse":
        return tf.keras.models.load_model(os.path.join(main_directory, 'modeller/upperbody/gomlek_kol/gomlek_kol_uzunluk_acc_train_099_val_089_incV3_299_299_weights.h5'))
    if model_name == "sweater":
        return tf.keras.models.load_model(os.path.join(main_directory, 'modeller/upperbody/kazak_yaka/kazak_yaka_tip_acc_train_098_val_070_incV3_299_299.h5'))
    if model_name == "skirt_length":
        return tf.keras.models.load_model(os.path.join(main_directory, 'modeller/lowerbody/skirt_length/skirt_length_incV3_299_299.h5'))   
    if model_name == "skirt_opening":
        return tf.keras.models.load_model(os.path.join(main_directory, 'modeller/lowerbody/skirt_opening/skirt_opening_acc_train_099_val_093_incV3_299_299.h5'))   
...
def predict_image(image_path, predictor, cfg,save_dir=None, show=False):
    ...
    if save_dir != None:
        ...
        if not os.path.exists(newpath):
            os.makedirs(newpath)
        fig.savefig(os.path.join(newpath, image_path.split("/")[-1].split(".")[0]+'_result.jpg'))
        filename = os.path.join(newpath, image_path.split("/")[-1].split(".")[0]+'.jpg') 
